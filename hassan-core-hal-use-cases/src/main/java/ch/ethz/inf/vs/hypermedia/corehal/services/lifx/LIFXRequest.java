...
abstract class LIFXRequest {
	...
	private byte[] generateHeader() {
		...
		// byteBuffer.put(new byte[]{(byte) 0x00, (byte) 0x34}); // For all
		// addressing
		byteBuffer.put(new byte[] { (byte) 0x00, (byte) 0x14 }); // For
																	// individual
																	// addressing
		...
		// Set sequence number to zero (we are not processing and creating
		// responses)
		byteBuffer.put((byte) 0x00);
		//---------------------------------------------------------//
